...
namespace ProjectPlatformer.Networking
{
    public class NetworkClient
    {
        ...
        public void DrawPlayer(SpriteBatch batch, NetworkPlayer player)
        {
            if (connectedClients.Count > 0)
            {
                try
                {
                    player.position = new Vector2(player.xPos, player.yPos); // see here if this works

                    int i = clients.IndexOf(player.playerIndex);
                    ...
                    {
                        playerPos = SmoothPlayerPosition(new Vector2(previousClientStates[i].xPos, previousClientStates[i].yPos), new Vector2(player.xPos, player.yPos), lerpProgress[i]);
                    ...
                    /*foreach (NetworkPlayer p in connectedClients)
                    {
                        ...
                        p.Draw(batch, playerPos);
                        //batch.Draw(p.Sprite, new Vector2(playerPos.X, playerPos.Y), null, Color.White, 0f, p.pivot, Vector2.One, p.flip, 0f); //change the sprite
        ...
        public void OnReceivePlayer(PacketHeader header, Connection connection, NetworkPlayer p)
        {
            ...
            Cell cell = connectedClients[i].cell;
            connectedClients[i] = p;
            connectedClients[i].cell = cell;

            Cell newCell = Cell.GetCell(Cell.SnapToGrid(new Vector2(p.xPos, p.yPos)));

            if (newCell != connectedClients[i].cell)
            {
                if (connectedClients[i].cell.playerEntities.Any(pe => pe.playerIndex == p.playerIndex)) // could possibly remove (may be needed the first time called)
                {
                    connectedClients[i].cell.playerEntities.RemoveAll(pe => pe.playerIndex == p.playerIndex);
                    Cell.SetGridCell(connectedClients[i].cell);
                }

                connectedClients[i].cell = newCell;
                connectedClients[i].cell.playerEntities.Add(connectedClients[i]);
                Cell.SetGridCell(connectedClients[i].cell);
            }
        }
